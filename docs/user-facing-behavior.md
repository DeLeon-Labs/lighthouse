...
- can display a named Focus as a single flattened pane or as split Sources,
  Work, and Unfiled panes without changing underlying membership;
- shows add actions in empty Sources and Work panes so notes and folders can be
  selected directly while inside a named Focus;
